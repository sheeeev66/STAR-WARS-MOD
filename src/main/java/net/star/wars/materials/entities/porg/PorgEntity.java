...
public class PorgEntity extends AnimalEntity implements Flutterer {
    ...
    public static DefaultAttributeContainer.Builder createPorgAttributes() {
        return MobEntity.createMobAttributes().add(EntityAttributes.GENERIC_MAX_HEALTH, 6.0D)
                .add(EntityAttributes.GENERIC_FLYING_SPEED, 0.4D).add(EntityAttributes.GENERIC_MOVEMENT_SPEED, 0.2D);
    }
    ...
    private void flapWings() {
        ...
        this.maxWingDeviation = (float) ((double) this.maxWingDeviation
                + (double) (!this.onGround && !this.hasVehicle() ? 4 : -1) * 0.3D);
    ...
    static {
        BREEDING_INGREDIENT = Ingredient.ofItems(Items.TROPICAL_FISH, Items.SALMON, Items.COOKED_SALMON, Items.COD,
                Items.COOKED_COD);
    }
}